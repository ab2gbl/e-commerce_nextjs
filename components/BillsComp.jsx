...
export default function BillsComp() {
  ...
  const filteredAndSortedBills = useMemo(() => {
    ...
    return sortedBills;
  }, [bills, filterType, searchProduct, dateFrom, dateTo, priceFrom, priceTo, sortBy, sortOrder]);
  ...
  const hasActiveFilters = filterType !== "all" || searchProduct || dateFrom || dateTo || priceFrom || priceTo;
                  ...
                  {filteredAndSortedBills.map((bill) => (
                    <tr key={bill.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(bill.type)}`}>
